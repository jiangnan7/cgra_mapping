...
size_t FastRouter::CacheCapacity = 1024 * 16;
                ...
                const std::unordered_map<std::string, std::unordered_map<std::string, std::vector<std::string>>> &pathsGiven)
{
    ...
    while(!queEdges.empty())
    {
        ...
        assert(!signal.empty()); 
        if((times++) >= 32*edgesToMap.size() || tried.count(edge.first + "->" + edge.second) > 8) //PARAM
        {
            failed = true;
            // clog << "FastRouter: Failed to route: "  << edge.first << "->" << edge.second << endl;
            break;
        }
        ...
        clog << "FastRouter: Routing: "  << edge.first << "->" << edge.second << endl;
        ...
        string pathname = edge.first + "->" + edge.second;
        ...
        if (tried.find(edge.first + "->" + edge.second) != tried.end()) {
        ...
        { 
            for(auto iter = _usedNode.find(vertex); iter != _usedNode.end(); iter = _usedNode.find(vertex))
            {
                ...
                string linkName = link.first + "->" + link.second; 
        ...
        tried.insert(edge.first + "->" + edge.second); 
...
bool FastRouter::search_route(const std::vector<std::pair<std::string, std::string>> &edgesToMap, const std::vector<std::string> &edgesSignal, 
                const std::unordered_map<std::string, std::unordered_map<std::string, std::vector<std::string>>> &pathsGiven)
{
    assert(edgesToMap.size() == edgesSignal.size());
    ...
    while(!queEdges.empty())
    {
        ...
        pathCandidates = _RRG.findPaths(edge.first, edge.second, funcPath); //Find ALL Paths
        _cache.insert({pathname,pathCandidates});
        cout << "pathCandidates " << pathCandidates.size() << endl;
    }
    return 0; 
}

bool FastRouter::norm_route(const std::vector<std::pair<std::string, std::string>> &edgesToMap, const std::vector<std::string> &edgesSignal, 
                const std::unordered_map<std::string, std::unordered_map<std::string, std::vector<std::string>>> &pathsGiven)
{
    assert(edgesToMap.size() == edgesSignal.size());
    ...
    auto funcPath = [&](const Vertex &vertex){
        ...
        const string &device = vertex.getAttr("device").getStr();
        if(_forbiddened.find(name) != _forbiddened.end()
        || (device != "io" && _FUs.find(device) != _FUs.end())
    ...
    while(!queEdges.empty())
    {
        ...
        &&pathsGiven.find(edge.first)->second.find(edge.second) != pathsGiven.find(edge.first)->second.end()){
            vector<string> path = pathsGiven.find(edge.first)->second.find(edge.second)->second; // Use the Given Path
            if(conflicts(path) == 0){
                // clog << "Router: use the Given path. " << endl;
                if(_paths.find(edge.first) == _paths.end())
                {
                    _paths[edge.first] = unordered_map<string, vector<string>>(); 
                }
        ...
        {   // Route Convenient
            ...
            // cout << pathConvenient.first << endl;
            _paths[edge.first][edge.second] = pathConvenient.first;
            for(const auto &node: pathConvenient.first)
            {
                _usedNode.insert({node, edge}); 
...
bool FastRouter::norm_route1(const std::vector<std::pair<std::string, std::string>> &edgesToMap, const std::vector<std::string> &edgesSignal, 
                const std::unordered_map<std::string, std::unordered_map<std::string, std::vector<std::string>>> &pathsGiven)
{
    assert(edgesToMap.size() == edgesSignal.size());
    ...
    _FUs.erase("REG");_FUs.erase("MEM");
    auto funcPath = [&](const Vertex &vertex){
        const string &name = vertex.name();
        const string &device = vertex.getAttr("device").getStr();
        if(_forbiddened.find(name) != _forbiddened.end()
        || (device != "IO" && _FUs.find(device) != _FUs.end())
        || (device != "MEM" && _FUs.find(device) != _FUs.end())
        ){
            return false;
        }
        return true;
...
bool FastRouter::try_route(const std::vector<std::pair<std::string, std::string>> &edgesToMap, const std::vector<std::string> &edgesSignal, 
                    ...
                    std::unordered_map<std::string, std::string> &usedSignal, std::unordered_map<std::string, std::unordered_map<std::string, std::vector<std::string>>> &paths)
{
    ...
    while(!queEdges.empty())
    {
        ...
        {   // Route Convenient
            ...
            if(_paths.find(edge.first) == _paths.end())
            {
                paths[edge.first] = unordered_map<string, vector<string>>(); 
            }
            paths[edge.first][edge.second] = pathConvenient.first;
            for(const auto &node: pathConvenient.first)
            {
                usedNode.insert({node, edge}); 
                usedSignal[node] = signal;
            }
            continue; 
        }
        usedNode.clear();
        ...
        // vector<unordered_set<string>> usedNodeTemp;
        // vector<size_t> pathIndex;
        // for (const auto &path : pathCandidates) {
        //     usedNodeTemp.push_back(unordered_set<string>());
        //     pathIndex.push_back(pathIndex.size());
        //     for (const auto &vertex : path) {
        //         if (_usedNode.find(vertex) != _usedNode.end() && _usedSignal[vertex] != signal) {
        //             assert(_usedSignal.find(vertex) != _usedSignal.end());
        //             usedNodeTemp.back().insert(vertex);
        //         }
        //     }
        // }
        // sort(pathIndex.begin(), pathIndex.end(), [&](size_t a, size_t b) {
        //     if (usedNodeTemp[a].size() < usedNodeTemp[b].size()) {
        //         return true;
        //     } else if (usedNodeTemp[a].size() == usedNodeTemp[b].size()) {
        //         return pathCandidates[a].size() < pathCandidates[b].size();
        //     }
        //     return false;
        // });
        ...
        // }
        // vector<string>        &path = pathCandidates[index]; 
        // unordered_set<string> used =  usedNodeTemp[index];
        // //unordered_set<string> used = unordered_set<string>(path.begin(), path.end());
        ...
        //     paths[edge.first] = unordered_map<string, vector<string>>(); 
        // }
        // paths[edge.first][edge.second] = path;
        // for(const auto &node: path){
        //     usedNode.insert({node, edge}); 
        //     usedSignal[node] = signal;
        // }
...
std::pair<std::vector<std::string>, bool> FastRouter::get_distance(const std::string &from, const std::string &to)
{
    ...
    auto funcPathStrict = [&](const Vertex &vertex){
        ...
        const string &device = vertex.getAttr("device").getStr();
        if(_forbiddened.find(name) != _forbiddened.end()
        ||(_usedSignal.find(name) != _usedSignal.end()) ||_FUs.find(device) != _FUs.end()){
    ...
    // pair<vector<string>, bool> pathConvenient = _RRG.findPath(from, to, funcPathStrict); // Find the Convenient Path
    
    return {firstPath, flag}; 
}
