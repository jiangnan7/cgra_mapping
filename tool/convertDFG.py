...
def parseDFG(lines, dfgType): 
    ...
    if dfgType == "normel":
    ...
    elif dfgType == "morpher": 
        for line in vertexLines: 
            splited = line.split("[")
            vertexName = splited[0].strip().split("\"")[1]

            opcode = splited[1].split("=")[-1][:-2].split("\"")[1].strip().lower()
            vertices.add(vertexName)
            compat[vertexName] = opcode
            vertices.add(vertexName + ".out0")
            compat[vertexName + ".out0"] = opcode + ".out0"
        for line in edgeLines: 
            splited = line.split("->")
            fromVertex = splited[0].strip().split("\"")[1] + ".out0"
            prefix = splited[1].split("[")[0].strip().split("\"")[1]
            if not prefix in ports: 
                ports[prefix] = []
            index = str(len(ports[prefix]))
            ports[prefix].append(fromVertex)
    ...
    for vertex in vertices: 
        if not "." in vertex:  
            ...
            if compat[vertex] in set(["mul"]): 
                ...
                compats += vertex + " MEM" + "\n" 
            elif compat[vertex] in set(["ls", "cmp", "select", "cmerge", "movc", "ars"]): 
                compats += vertex + " ALU" + "\n"
...
if __name__ == "__main__": 
    ...
    content, compats, vertices, edges = parseDFG(lines ,dfgType)
    # print(content)
