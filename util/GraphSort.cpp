...
namespace CGRA
{
    namespace GraphSort
    {
        ...
        vector<string> sortTOPO(const Graph &graph){
            if(graph.vertices().empty()){
                return {};
            }
            vector<string> candiSeeds;
            ...
            unordered_set<string> verticesSorted;
            vector<vector<string>> candSort;
            for(const auto &cand: candiSeeds){
                vector<string> seed = {cand};
                vector<string> path = {cand};
                verticesSorted.emplace(cand);
                while (!seed.empty()){
                    bool flag = false;
                    vector<string> nodes = seed;
                    seed.clear();
                    size_t count = 0;
                    for(const auto &node: nodes){
                        // if(!graph.edgesIn(node).empty()){
                        //     count += 1;
                        // }
                        // if(count < nodes.size()){
                        //     flag = true;
                        // }
                        for(const auto &edge: graph.edgesIn(node)){
                            if(verticesSorted.find(edge.from()) == verticesSorted.end()){
                                path.emplace_back(edge.from());
                                seed.emplace_back(edge.from());
                                verticesSorted.emplace(edge.from());
                            }
                ...
                // if(flag){
                        candSort.emplace_back(path);
                        // break;
                    // } 
            }
            // cout << candSort << endl;
            vector<string> sort;
            for(const auto &cand: candSort){
                sort.insert(sort.end(), cand.begin(), cand.end());
            }
            // exit(0);
            // const string &seed = candiSeeds[rand() % candiSeeds.size()];
            return sort;
