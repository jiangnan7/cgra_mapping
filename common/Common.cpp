...
int manhattan_distance(const int p_clcye, const int p_x, const int p_y, 
                       const int t_clcye, const int t_x, const int t_y)
{
    int dx = p_x - t_x;
    int dy = p_y - t_y;
    int dc = p_clcye - t_clcye;
    return (abs(dx) + abs(dy) + abs(dc));
}

int manhattan_distance(const int p_x, const int p_y, 
...
std::unordered_map<std::string, std::vector<std::string>> readPred(const std::string &filename)
{
